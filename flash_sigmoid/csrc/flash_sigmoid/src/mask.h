...
struct Mask {
    ...
    __forceinline__ __device__ void apply_mask(Tensor<Engine, Layout> &tensor_,
        ...
        if constexpr (Need_masking) {
            ...
            // We always compute offsets using row and column indices both.
            const int lane_id = threadIdx.x % 32;
            const int col_idx_offset = col_idx_offset_ + (lane_id % 4) * 2;
            const int max_seqlen_diff = max_seqlen_k - max_seqlen_q;
            const int max_seqlen_diff_left = max_seqlen_diff - window_size_left;
            const int max_seqlen_diff_right = max_seqlen_diff + 1 + window_size_right;
            ...
            for (int mi = 0; mi < size<0, 1>(tensor); ++mi) {
                ...
                for (int i = 0; i < size<0, 0>(tensor); ++i) {
                    const int row_idx = row_idx_base + i * 8;
                    const int col_idx_limit_left = std::max(0, row_idx + max_seqlen_diff_left);
                    const int col_idx_limit_right = std::min(max_seqlen_k, row_idx + max_seqlen_diff_right);
                    ...
                    for (int nj = 0; nj < size<1, 1>(tensor); ++nj) {
                        ...
                        for (int j = 0; j < size<1, 0>(tensor); ++j) {
                            ...
                            if constexpr (Has_alibi) {
                                tensor(make_coord(i, mi), make_coord(j, nj)) -= alibi_slope * abs(row_idx + max_seqlen_diff - col_idx);
                            }
